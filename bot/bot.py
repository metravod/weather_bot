...
class SetMyCityWeather(StatesGroup):
    my_city = State()
...
async def get_user_city_weather(message):
    ...
    markup.add(btn1)
    text = 'Установите ваш город'
    await message.answer(text, reply_markup=markup)
    await SetMyCityWeather.my_city.set()


@dp.message_handler(state=SetMyCityWeather.my_city)
